...
Robot_Coordinate     = [3 2]               ;% Rabot Coordinate = [x-axis y-axis]
Obstacle1_Coordinate = [6 10]              ;% Obstacle1 Coordinate = [x-axis y-axis]
Obstacle2_Coordinate = [6 2]               ;% Obstacle2 Coordinate = [x-axis y-axis]
Goal_Coordinate      = [9 5]               ;% Target Coordinate = [x-axis y-axis]
Sensor_Range         = 2                   ;% Sensor Range being used
Step_Size   = 0.2*Sensor_Range             ;% Radius of Circle for Artificial Points
...
while(DTG > 0.6)
	...
	D_Obst1 = max(min_distance, sqrt((Robot_Coordinate(1)-Obstacle1_Coordinate(1))^2 + (Robot_Coordinate(2)-Obstacle1_Coordinate(2))^2));
	D_Obst2 = max(min_distance, sqrt((Robot_Coordinate(1)-Obstacle2_Coordinate(1))^2 + (Robot_Coordinate(2)-Obstacle2_Coordinate(2))^2));
	D_Goal = max(min_distance, sqrt((Robot_Coordinate(1)-Goal_Coordinate(1))^2 + (Robot_Coordinate(2)-Goal_Coordinate(2))^2));

	% Calculate the potential
	J_ObstT = Obstacle(1) / D_Obst1 + Obstacle(1) / D_Obst2;
	J_GoalT = -Goal(1) / D_Goal;
	% Calculate the overall potential and constrains its maximum value
	max_JT = 0.3; % maximum overall potential
	JT = min(J_ObstT + J_GoalT, max_JT);
...
for i=1:n
    D_Obst1_Bacteria = max(min_distance, sqrt((Bacteria_x(i)-Obstacle1_Coordinate(1))^2 + (Bacteria_y(i)-Obstacle1_Coordinate(2))^2));
    D_Obst2_Bacteria = max(min_distance, sqrt((Bacteria_x(i)-Obstacle2_Coordinate(1))^2 + (Bacteria_y(i)-Obstacle2_Coordinate(2))^2));
    D_Goal_Bacteria = max(min_distance, sqrt((Bacteria_x(i)-Goal_Coordinate(1))^2 + (Bacteria_y(i)-Goal_Coordinate(2))^2));

    J_ObstT_Bacteria(i) = Obstacle(1) / D_Obst1_Bacteria + Obstacle(1) / D_Obst2_Bacteria;
    J_GoalT_Bacteria(i) = -Goal(1) / D_Goal_Bacteria;
